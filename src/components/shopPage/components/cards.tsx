  ...
  return (
    ...
    {products.map((product, index) => (
      <div className="flex flex-col gap-6 w-full" key={index}>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] gap-5 p-0 w-full box-border">
          {product.cards.map((card) => (
            <div
              key={card.id}
              className=" px-2 cursor-pointer w-full bg-white transition duration-300 ease-in-out"
